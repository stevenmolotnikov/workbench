...
export function ActivationPatching({
    ...
    const [chartData, setChartData] =
        useState<ActivationPatchingResponse | null>(demoChartData);
    ...
    const [source, setSource] = useState<Completion>(
        makeDefaultCompletion("source")
    );
    const [destination, setDestination] = useState<Completion>(
        makeDefaultCompletion("destination")
    );
    ...
    const handleRun = async () => {
        ...
        try {
            const response = await fetch(
                config.getApiUrl(config.endpoints.patch),
                {
                    method: "POST",
                    headers: {
